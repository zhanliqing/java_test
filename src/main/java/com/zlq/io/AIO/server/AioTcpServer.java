 ...
 * 异步的。两者使用时，都用到了同样的异步通道管理器，异步通道管理器通过线程池管理。 异步通道管理器，可以生成服务端socket与客户端socket。 *
 * 使用服务端socket或客户端socket都需要一个操作处理器（CompletionHandler）
 *
 * 当有信息时异步通道管理器会把 相关信息传递给操作作处理器。 * 操作处理器的方法是同一方法，但方法的参数是泛型，随着调用它的方法不同而改变。
